...
const getChannelStats = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (totalLikes.length === 0){
            throw new ApiError(404, "There is no likes on any video.")
        }
            
        if (totalVideos.length === 0) {
            throw new ApiError(404, "There is no video in the channel.")
        }
        
        if (totalVideoViews.length === 0) {
            throw new ApiError(404, "No views on your videos.")
        }
        
        return res
            .status(200)
            .json(
                new ApiResponse(200, [countSubs, totalVideos, totalVideoViews, totalLikes],
